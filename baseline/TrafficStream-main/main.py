    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, data in enumerate(train_loader):
            if epoch == 0 and batch_idx == 0:
                ...
                args.logger.info("sub_adj {}".format(args.sub_adj.shape))
            data = data.to("cuda:0", non_blocking=pin_memory)
...
def main(args):
    ...
    for year in range(args.begin_year, args.end_year+1):
        ...
        if year > args.begin_year and args.strategy == "incremental":
            # Load the best model
            model, _ = load_best_model(args)
            args.logger.info("[*] Loaded previous year's model for year {}".format(year))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type = str, default = "PEMSD3-stream")
    ...
    # parser.add_argument("--first_year_model_path", type = str, default = "res/PEMSD3-stream/trafficStream2024-12-28-21:58:32.828944/2011/16.982.pkl", help='specify a pretrained model root')
    # parser.add_argument("--first_year_model_path", type = str, default = "res/PEMSD4-stream/trafficStream2024-12-28-23:21:21.381139/2009/25.3532.pkl", help='specify a pretrained model root')
    parser.add_argument("--first_year_model_path", type = str, default = "res/PEMSD8-stream/trafficStream2024-12-29-15:05:48.920796/2012/19.2734.pkl", help='specify a pretrained model root')
    args = parser.parse_args()
    # args.conf = f"config/{args.dataset}.json"
    init(args)
    seed_set(0)
